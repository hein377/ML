...
#NUM_TREES, NUM_INSTANCES, NUM_ATTRIBUTES = 5, 50, int(math.sqrt(len(IRIS_PVAL.keys())-1))
NUM_TREES, NUM_INSTANCES, NUM_ATTRIBUTES = 5, 50, 3
...
# DECISION TREE - RANDOMFOREST CLASSIFICATION
...
def create_newdf(df, attributename, val):
    att_ind, rows_toremove = list(df.columns).index(attributename), []
...
def find_best_attribute(df, cur_entropy, feature_set):
    if(len(feature_set) == 1):
        return (attname:=next(iter(feature_set))), pos_values(df[attname])       #if there is only 1 attribute left, return that attribute and its pvals
    att_entropygain = {}
    for attname in feature_set:
        attpvals = pos_values(df[attname])
        if(attname != df.columns[-1]): att_entropygain.update({attname:calc_entropygain(df, attname, attpvals, cur_entropy)})
    return (attname:=choose_highestval_key(att_entropygain)), pos_values(df[attname])
...
def randomize_df(df, ninstances, nattributes):
    nrows, ncols = df.shape[0]-1, df.shape[1]-1             #max row index, max col index excluding class label col
    random_instance_inds, random_att_inds = [random.randint(0,nrows) for i in range(ninstances)], random.sample(range(0,ncols), nattributes)+[df.shape[1]-1]
    new_df = df.iloc[:,random_att_inds]         #split by selected random_att_inds list
    new_df = new_df.iloc[random_instance_inds]                      #split by selected random_instances_inds list
    return new_df

def create_RandomForestModel(df, ntrees, ninstances, nattributes):
    dtrees = []
    for i in range(ntrees): dtrees.append(create_dtree_model(randomize_df(df, ninstances, nattributes)))
    return dtrees
...
def choose_cnode(cnodesls, instanceAttVal):
    correct_cnode = None
    for cnode in cnodesls:
        if str(instanceAttVal) in cnode.name: 
            correct_cnode = cnode
            break
    return correct_cnode
...
def treeclassify(tmodel, instance):                 #instance = {attname1:att1val, attname2: att2val, ...}
    ...
    while(len(cnodesls:=root.children) != 0):
        ...
        cnode = choose_cnode(cnodesls, instanceAttVal)
        if(cnode == None): return choose_label({}, 1)              #if the path specified does not exist in the tree model, return random class label w/ all equal prob
        else: root = cnode

    label_prob = json.loads((s:=root.name)[s.index(">")+1:])
    return choose_label(label_prob, 1)

def random_forest_classify(tmodels, instance):
    predictions = []
    for tree in tmodels: predictions.append(treeclassify(tree, instance))
    return max(set(predictions), key=predictions.count)                             #return mode of predictions

def test_randomforest_model(tmodels, df):
    cmatrix = np.zeros(((n:=len(LABEL_NUM.keys())), n))               #n=num_instances
    for index, row in df.iterrows():
        model_prediction, true_label_val = random_forest_classify(tmodels, row.to_dict()), row[-1]
...
def random_forest(training_df, testing_df):
    dtrees = create_RandomForestModel(training_df, NUM_TREES, NUM_INSTANCES, NUM_ATTRIBUTES)
    cmatrix, testing_accRate = test_randomforest_model(dtrees, testing_df)

    print("RANDOMFOREST", end="\n")
    print("Random Forest Tree Models:")
    for tree in dtrees: print_tree(tree)
...
random_forest(training_df, testing_df)
...
randomforest_lib(training_df, testing_df)