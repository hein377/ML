...
def hadamard_transform_input_registers(circ_name, num_quantumregisters, num_qubits):
    for i in range(num_quantumregisters*num_qubits):
        eval("{}.h({})".format(circ_name, i))
...
def initialize_quantumcircuit(num_quantumregisters, num_qubits, num_ancillary_bits, vec_dimension, network, vec_parameter, vec_parameter_binls):
    '''print(f"{num_ancillary_bits}     shape: {np.shape(num_ancillary_bits)}", end = "\n\n")
    print(f"{vec_dimension}     shape: {np.shape(vec_dimension)}", end = "\n\n")
    print(f"{vec_parameter}     shape: {np.shape(vec_parameter)}", end = "\n\n")
    print(f"{vec_parameter_binls}     shape: {np.shape(vec_parameter_binls)}", end = "\n\n")
    input()'''
...
def create_phase_oracle(cost_function, input_a, register_num, big_n):    #register_num starts index at 0
    phase_oracle_matrix = np.zeros(shape=(big_n,big_n), dtype=np.complex_)
    for i in range(big_n): phase_oracle_matrix[i,i] = cmath.exp(2*pi*big_n*cost_function(input_a+INTEGER_GRIDVAL[i][0])*complex(0,1))
...
def apply_phase_oracle(circ, circuit, cost_function, input_a, register_num, n, big_n):    #register_num starts index at 0
    phase_oracle = create_phase_oracle(cost_function, input_a, register_num, big_n)
...
def calc_quantum_gradient(d, n, big_n, a_vec, dot_vec, dot_vecs_ls, a_vecs_ls, expected_output, network):
    ...
    circuit.barrier()
    for register_num in range(d): apply_phase_oracle(circ, circuit, calc_error, a_vec, register_num, n, big_n)
    circuit.barrier()
...
def back_propagation(actual_table, network):            #network = [ layer1, layer2, ... ]; layer1 = [ weight_matrix, bias_scalar ]              #actual_table = [ ( input_array <1x784 np.array of ints>, output_array <1x10 np.array of ints> ), ... ]
    ...
    while(num_epoch < MAX_NUM_EPOCHS):
        ...
        print(f"Epoch{num_epoch}: {cur_network_accuracy}")
        ...
        lamb *= 0.99
        num_epoch += 1
    return network, total_testing_time
...
trained_network, total_testing_time = back_propagation(training_actual_table, neural_network)
total_train_end = time.perf_counter()
actual_training_time = ((total_train_end-total_train_start) - total_testing_time)
print(actual_training_time)